...
class GameState:
    ...
    def get_power_damage_multiplier(self):
        ...
        # S'assurer que les dégâts ne tombent pas en dessous d'un seuil minimal (ex: 10%)
    ...
    def handle_kamikaze_impact(self, kamikaze_enemy, generator_hit, missed=False):
        ...
        if generator_hit and generator_hit.active and not missed:
            ...
            generator_hit.active = False  # Le marquer inactif
            # Optionnel: le retirer de self.buildings si vous ne voulez plus le garder du tout
            ...
            if "ruin" in objects.BUILDING_STATS:
                ...
                self.buildings.append(ruin_obj)  # Ajouter la ruine à la liste des bâtiments à dessiner
            ...
            # 4. TODO: Effet d'explosion pour le kamikaze/générateur
            #    Peut être créé ici et ajouté à self.particle_effects
