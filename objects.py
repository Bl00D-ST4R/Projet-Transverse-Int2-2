...
BUILDING_STATS = {
    "frame": { # NOUVEAU TYPE - Structure de base sur laquelle construire
        cfg.STAT_COST_MONEY: 10, # Coût très bas pour la structure
        cfg.STAT_SPRITE_DEFAULT_NAME: "frame.png",
    },
    "fundations": { # NOUVEAU TYPE - Fondation renforcée de départ (non-constructible par le joueur?)
        cfg.STAT_COST_MONEY: 0, # Pré-placé ou coût spécial
        cfg.STAT_SPRITE_DEFAULT_NAME: "fundations.png", # Assurez-vous que ce nom de fichier est correct
        "is_reinforced": True # Flag ou propriété pour identifier
    },
    "generator": { # Peut être placé sur 'frame'
        cfg.STAT_COST_MONEY: 150,
        cfg.STAT_POWER_PRODUCTION: 10, # Produit 10W
        cfg.STAT_SPRITE_DEFAULT_NAME: "battery.png",
    },
    "miner": { # Peut être placé sur 'fundations' ou autre 'miner'
        ...
        cfg.STAT_SPRITE_VARIANTS_DICT: {
            "single": "miner.png",
            "stacked_bottom": "miner_stacked_bottom.png", # Placeholder
            "stacked_middle": "miner_stacked_middle.png", # Placeholder
            "stacked_top": "miner_stacked_top.png", # Placeholder
        },
        cfg.STAT_SPRITE_DEFAULT_NAME: "miner.png",
    },
    "storage": { # Peut être placé sur 'frame'
    ...
    # NOTE: L'ancien type "foundation" a été remplacé par "frame" et "fundations".
    # S'assurer que toute référence à "foundation" dans le code est mise à jour.
}

TURRET_STATS = {
    "gatling_turret": { # Peut être placé sur 'frame'
        ...
        cfg.STAT_TURRET_BASE_SPRITE_NAME: "turret_base_placeholder.png", # Placeholder
        cfg.STAT_TURRET_GUN_SPRITE_NAME: "gun_sketch.png",
    },
    "mortar_turret": { # Peut être placé sur 'frame'
...
PROJECTILE_STATS = {
    "bullet": {
        ...
        cfg.STAT_SPRITE_DEFAULT_NAME: "bullet.png", # Placeholder
        cfg.STAT_PROJECTILE_LIFETIME_SEC: 3.0,
    },
    "mortar_shell": {
        cfg.STAT_DAMAGE_AMOUNT: 50,
        cfg.STAT_AOE_RADIUS_PIXELS: 50,
        cfg.STAT_SPRITE_DEFAULT_NAME: "mortar_shell.png", # Placeholder
...
ENEMY_STATS = {
    ...
    2: { # Ennemi Rapide
        ...
        cfg.STAT_SPRITE_DEFAULT_NAME: "enemy_fast.png", # Placeholder
    ...
    3: { # Ennemi Tank
        ...
        cfg.STAT_SPRITE_DEFAULT_NAME: "enemy_tank.png", # Placeholder
...
# Fonction pour vérifier si c'est une structure de base (frame)
def is_frame_type(item_type_string):
    return item_type_string == "frame"

# Fonction pour vérifier si c'est une fondation renforcée
def is_reinforced_foundation_type(item_type_string):
     return item_type_string == "fundations"
...
class Building(GameObject):
    ...
    def __init__(self, building_type, grid_pos_tuple): # (row, col)
        ...
        self.is_reinforced = self.stats.get("is_reinforced", False) # Lire le flag
        ...
        # Calcul de la position pixel (devrait être fait dans game_functions idéalement)
        # Utiliser des valeurs par défaut si cfg.GRID_OFFSET_Y n'est pas défini ou si game_state n'est pas passé
        offset_x = getattr(cfg, 'GRID_OFFSET_X', 50)
        offset_y = getattr(cfg, 'GRID_OFFSET_Y', 300) # Valeur par défaut si non définie
        pixel_pos = util.convert_grid_to_pixels(self.grid_pos, (offset_x, offset_y))
    ...
    def update_sprite_based_on_context(self, game_grid_ref):
        # ... (logique existante pour les mineurs empilés inchangée) ...
        ...
        else: # Fallback sur le sprite par défaut si la clé n'existe pas ou sprite non chargé
            default_sprite_name = self.stats.get(cfg.STAT_SPRITE_DEFAULT_NAME, "placeholder.png")
            self.original_sprite = util.load_sprite(cfg.BUILDING_SPRITE_PATH + default_sprite_name)

        self.sprite = util.scale_sprite_to_tile(self.original_sprite)
        # Recalculer le rect si la taille du sprite change ? Normalement scale_sprite_to_tile garde la taille.
...
class Turret(GameObject):
    # ... (le __init__ et les autres méthodes restent inchangés, ils utilisent déjà TURRET_STATS) ...
    _id_counter = 0
    def __init__(self, turret_type, grid_pos_tuple):
        ...
        offset_x = getattr(cfg, 'GRID_OFFSET_X', 50)
        offset_y = getattr(cfg, 'GRID_OFFSET_Y', 300)
        pixel_pos = util.convert_grid_to_pixels(self.grid_pos, (offset_x, offset_y))
        ...
        if self.gun_sprite_scaled_original:
            ...
            self.gun_pivot_offset = (cfg.TILE_SIZE // 2, cfg.TILE_SIZE // 4)

        self.target_enemy = None
        self.current_angle_deg = 0
        self.is_internally_active = True

    # ... (set_active_state, find_target, update, shoot, draw - inchangés) ...
    def set_active_state(self, is_powered):
        self.is_internally_active = is_powered
...
class Projectile(GameObject):
    ...
    def __init__(self, projectile_type, origin_xy_pixels, angle_deg, initial_vx=None, initial_vy=None):
        ...
        if self.sprite:
            ...
            self.rect = pygame.Rect(origin_xy_pixels[0]-5, origin_xy_pixels[1]-5, 10, 10)
    ...
    def on_hit(self, game_state_ref):
        ...
        if self.is_mortar_shell and self.aoe_radius > 0:
            if hasattr(game_state_ref, 'trigger_aoe_damage'):
...
class Enemy(GameObject):
    ...
    def __init__(self, initial_pos_xy_ref, enemy_type_id, variant_data=None):
        ...
        if self.original_sprite:
            ...
            default_size = cfg.scale_value(30)
...
def calculate_mortar_fire_solution(turret_pos_pixels, target_pos_pixels, projectile_initial_speed_pixels, gravity_pixels_s2):
    ...
    try:
        # Division par g * dx_pixels peut lever une erreur si g ou dx est 0 (dx déjà géré)
        if g == 0: return None # Gravité nulle, trajectoire droite
        tan_theta_high = (v0**2 + sqrt_discriminant) / (g * dx_pixels)
    except ZeroDivisionError:
        return None # Devrait être géré par la vérification dx plus haut
    ...
    try:
        ...
        return None # Si v0 ou cos_chosen_angle est 0
        ...
        if animation_frames_list:
            for f_original in animation_frames_list:
                if isinstance(f_original, pygame.Surface):
                    # Assumer que les frames sont déjà à la bonne taille ou les scaler ici si besoin
                    # self.frames.append(util.scale_sprite_to_some_size(f_original))
                    self.frames.append(f_original) # Pour l'instant, utilise les frames fournies telles quelles
